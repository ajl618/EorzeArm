<template>
        ...
        <div class="max-w-md z-80">
          ...
          <form @submit.prevent="handleSubmit" class="bg-white shadow-md rounded-3xl p-8 w-[380px]" autocomplete="on">
            ...
            <div class="mb-4 text-purple-900 border-2 border-purple-200 rounded-lg">
              <input v-model="username" name = "username" id = "username" type="text" placeholder="Usuario" required  class="w-full rounded-lg"/>
            </div>
  
            <div class="mb-4 text-purple-900 border-2 border-purple-200 rounded-lg">
              <input v-model="email" name = "email" id = "email" type="email" placeholder="Email" required  class="w-full rounded-lg"/>
            </div>

            <div class="mb-6 text-purple-900 border-2 border-purple-200 rounded-lg">
              <input v-model="password" name = "password" id="password" type="password" placeholder="Contraseña" required class="w-full rounded-lg"/>
            </div>

            <div class="mb-6 text-purple-900 border-2 border-purple-200 rounded-lg">
              <input v-model="password_repeat" name = "password_repeat" id="password_repeat" type="password" placeholder="Repita su contraseña" required class="w-full rounded-lg"/>
            </div>
  ...
  function handleSubmit() {
    ...
    userControllerStore.create(data).then(success => {
      ...
      appStore.setIdle().then(function(){
        appStore.showAlert('error', '!Error!', error.data.mensaje, 'Cerrar');
        });
    });
}
